...
- **Render**: trusted API, service-role Supabase access, protected job execution endpoints, notification delivery, webhooks, Cloudinary signing.
- **Vercel**: frontend hosting and public/admin UI delivery.

The Firebase files listed here are archived under `legacy/firebase-production-archive/` for reference only. New runtime code must not import Firebase SDKs or deploy through Firebase.
...
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| `legacy/firebase-production-archive/functions/index.js`                    | Firebase callable functions, Firestore triggers, scheduled functions, notification logic, admin/security workflows. | Reference for backend business behavior.                                |
| `legacy/firebase-production-archive/functions/client-config.js`            | White-label business name, contact notification email, timezone, Cloudinary folder.                                 | Reference for `site_settings`, Render env vars, and/or tenant settings. |
| `legacy/firebase-production-archive/functions/waitlist-action-messages.js` | Waitlist slot-occupied error reason/message.                                                                        | Reference for Render error codes and API error details.                 |

---

## Backend function export parity matrix

Last verified in code on **2026-06-11** against archived `legacy/firebase-production-archive/functions/index.js` and the current `backend/src`, browser adapters, and `render.yaml` implementation.

Latest local validation on **2026-06-11**: `npm run test:phase9` passed (`check:js`, root unit tests 13/13, backend tests 99/99 across 21 files). Full `npm test` also passed, including Playwright E2E 23/23. The booking/waitlist/reminder/expired-slot parity fixes are covered by regression tests. Items marked partial or with conditional notes below still need either implementation or explicit product sign-off before final Firebase parity closure.
...
| `clientGetWaitlistQueueInfo`             | Authenticated client fetches queue position for own booking/waitlist entry.                                                                                      | Render endpoints `GET /api/v1/bookings/:bookingId/waitlist-queue` and `GET /api/v1/waitlist/:waitlistId/queue`; both require ownership and recalculate queue. Browser adapter chooses booking route when `bookingId` is provided, otherwise waitlist route.           | Parity fixed                          | 2026-06-11 coverage confirms owner access, non-owner 403, missing waitlist 404, `booking.waitlist_id` and `booking_id` lookup paths, and adapter route selection.                                                                                                                                                         |
...
| `clientReleaseExpiredBookingSlot`        | Authenticated client can request expired slot release by slot ID.                                                                                                | Render endpoints `POST /api/v1/booking-slots/:slotId/release-expired` for Supabase UUIDs and `POST /api/v1/booking-slots/legacy/:legacySlotId/release-expired` for Firebase-era `date__stylist__time` IDs. Adapter chooses route by UUID detection.                   | Parity fixed                          | 2026-06-11 coverage confirms UUID-vs-legacy adapter routing, legacy time normalization (`09:00`, `9:00`, `9 AM`, `9:00 AM` style forms), no-match/invalid/not-expired outcomes, and reuse of the existing expired release workflow. Authenticated-only like Firebase callable; no ownership check, matching old behavior. |
...
| `notifyWaitlistOnSlotOpen`               | Booking slot delete trigger notifies first waiting waitlist entry by email/WhatsApp and marks status `notified`/`notification_failed`.                           | Slot release service and external scheduled job queue slot-open notifications via outbox and update waitlist notification state.                                                                                                                                      | Implemented via outbox                | Render releases slots by row update (`taken=false`, `released_at`) rather than delete; `syncWaitlistSlotOpenNotifications` is scheduled every 15 minutes.                                                                                                                                                                 |
...
| `bookingSlots`         | `booking_slots`                                              | Use unique constraint for tenant/date/time/stylist and prevent double-booking. Prefer updating/releasing rows over deleting if audit history is needed. Public reads are served by unauthenticated `GET /api/v1/booking-slots`, default to future `taken=true` rows, poll at 5000ms in the browser adapter, and intentionally expose only non-private slot fields (`id`, tenant/date/time/start/end/stylist/taken/timestamps). |
...
Additional target tables from the rewrite plan that do not have a direct single Firebase trigger in `legacy/firebase-production-archive/functions/index.js` but are needed for the rebuilt app:
...
Legacy `legacy/firebase-production-archive/functions/client-config.js` currently contains:
